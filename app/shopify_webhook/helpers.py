...
def get_proxy_signature(query_dict, secret):
    ...
    for key in sorted(query_dict.keys()):
        sorted_params += "{}={}".format(key, ",".join(query_dict.getlist(key)))

    signature = hmac.new(
        secret.encode("utf-8"), sorted_params.encode("utf-8"), hashlib.sha256
    )
...
def proxy_signature_is_valid(request, secret):
    ...
    try:
        return hmac.compare_digest(
            calculated_signature.encode("utf-8"), signature_to_verify.encode("utf-8")
        )

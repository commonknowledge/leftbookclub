...
from . import signals
from .decorators import app_proxy, webhook
...
class WebhookView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            signals.webhook_received.send_robust(
                self,
                domain=request.webhook_domain,
                topic=request.webhook_topic,
                data=request.webhook_data,
            )
            getattr(signals, signal_name).send_robust(
                self,
                domain=request.webhook_domain,
                topic=request.webhook_topic,
                data=request.webhook_data,
...
class LiquidTemplateView(TemplateView):
    ...
    content_type = getattr(
        settings, "LIQUID_TEMPLATE_CONTENT_TYPE", "application/liquid; charset=utf-8"
    )
